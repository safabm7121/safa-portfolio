...
const CARD_GLB = '/model/card.glb';
const LANYARD_TEXTURE = '/model/lanyard.png';
...
export default function Lanyard({
  ...
  useEffect(() => {
    ...
    // ⭐ Force reload by adding a key change
...
function Band({
  ...
  const getLerped = (body: LanyardRigidBody | null): THREE.Vector3 => {
  ...
  const gltfResult = useGLTF(CARD_GLB);
  const nodes = gltfResult.nodes;
  const materials = gltfResult.materials;
  ...
  // ⭐ FALLBACK when GLB fails - ALWAYS show something
  if (!nodes || !nodes.card) {
    return (
      <group position={[0, 4, 0]}>
        <RigidBody ref={fixed} type="fixed" />
        <RigidBody position={[2, 0, 0]} ref={card} type="dynamic">
